...
public final class TosWalletAPIAssembly {
    ...
    public var api: TosWalletAPI {
        TosWalletAPIImplementation(
            urlSession: .shared,
    ...
    var apiV1ConfigURL: URL? {
        guard
            ...
            let toswalletApiUrl = configuration.mainnet.toswalletApiUrl
        else {
            return nil
        }

        return URL(string: toswalletApiUrl)
    }

    var apiV1DefaultURL: URL {
        URL(string: "https://api.tos.network")!
    }
}
